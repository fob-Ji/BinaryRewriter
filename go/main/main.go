...
func voidTrail (index int ,hexStr string) string{
	return hexStr[index:len(hexStr)-1]
}
...
func binRewrite(hexStr string) string {
	...
	defer func(){
		e := recover()
		if e != nil { // outOfidex 일때
			reWritedStr = reWritedStr + voidTrail(index, hexToken) + opcodes.String(opcodes.JUMPDEST) + opcodes.String(opcodes.PUSH2) +fallbackOperand+ opcodes.String(opcodes.JUMPI)
		}
	}()
	return reWritedStr
}
