...
func dasm_printer(hexStr string) {
	...
	for index < len(hexStr) {
		...
		opCodeToken := OpCode(hexToken[0]) // byte to OpCode(struct)
		var operator string = opCodeToString[opCodeToken] // operator

		switch {
		case IsPush(opCodeToken): // operator is Push
		...
		case len(opCodeToString[opCodeToken]) == 0:
			fmt.Printf("0x%x \n",hexToken)
			index += 2
